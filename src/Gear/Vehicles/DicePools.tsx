...
export const DroneEvasion: FC<VehiclePoolProps> = () => {
  ...
  const evasion: number = evasionAutosoft
    ? evasionAutosoft.attributes[AutosoftAttr.rating]
    : -1
...
export const DroneManeuvering: FC<VehiclePoolProps> = () => {
  ...
  const maneuvering: number = maneuveringAutosoft
    ? maneuveringAutosoft.attributes[AutosoftAttr.rating]
    : -1
...
export const DroneStealth: FC<VehiclePoolProps> = () => {
  ...
  const stealth: number = stealthAutosoft
    ? stealthAutosoft.attributes[AutosoftAttr.rating]
    : -1
...
export const DronePerception: FC<VehiclePoolProps> = () => {
  ...
  const clearsight: number = clearsightAutosoft
    ? clearsightAutosoft.attributes[AutosoftAttr.rating]
    : -1
...
export const DroneCracking: FC<VehiclePoolProps> = () => {
  ...
  const warfare: number = warfareAutosoft
    ? warfareAutosoft.attributes[AutosoftAttr.rating]
    : -1
...
export const DriverPiloting: FC<VehiclePoolProps> = ({ vehicle }) => {
  ...
  if (hasSpecialty(pilotingSkill, vehicle.pilotingSpeciality))
    groups.push({ name: vehicle.pilotingSpeciality, size: 2 })
  if (hasExpertise(pilotingSkill, vehicle.pilotingSpeciality))
    groups.push({ name: vehicle.pilotingSpeciality, size: 3 })

  const dmgPenaltyTypes = [
    DamageType.charPhysical,
    DamageType.charStun,
    DamageType.vehiclePhysical,
  ]
  return (
    <DicePool
...
export const RiggedPiloting: FC<VehiclePoolProps> = ({ vehicle }) => {
  const pilotingSkill = useActiveSkill(ActiveSkillIds.CRB.piloting)
  ...
  const controlRig = useFindGear<AugmentData>(
    (gear) => gear.augmentType === AugmentType.controlRig,
  )
