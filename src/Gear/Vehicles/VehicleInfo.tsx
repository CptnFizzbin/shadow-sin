...
export const VehicleInfo: FC<GearInfoProps<VehicleData>> = ({
  ...
  const vehicleMods = useAllNestedGear(vehicle.id).filter(isVehicleMod)

  const weapons = useAllNestedGear(vehicle.id).filter(isWeapon)

  if (vehicle.destroyed) {
    return <DestroyedVehicleInfo item={vehicle} />
  }

  let autosofts = allAutosofts.filter((gear) => gear.attachedTo === vehicle.id)
    ...
    <DamageProvider type={DamageType.vehiclePhysical} id={vehicle.id}>
      <AutosoftProvider autosofts={autosofts}>
        <GearInfoBlock item={vehicle}>
          <Stack
            gap={1}
            direction={mdScreenOrLarger ? "row-reverse" : "column"}
          >
            <Stack
              gap={1}
            ...
            >
              ...
              <Box>
                <DamageTrack
                  type={DamageType.vehiclePhysical}
                  max={physicalMax}
                  label="Physical"
                />
              </Box>
                      ...
                      {Object.entries(vehicle.modSlots).map(([type, slots]) => (
                        <VehicleSlots
                          ...
                          items={vehicleMods
                            ...
                            .filter(
                              (item) =>
                                item.attributes[VehicleModAttr.slotType] ===
                                type,
                            )
                            .map((item) => ({
                              id: item.id,
                ...
                <Box>
                  ...
                  <Paper variant="outlined" sx={{ padding: 1 }}>
                    <Stack
                      ...
                      divider={<Divider orientation="vertical" flexItem />}
                    >
                      {Object.entries(vehicle.hardpoints).map(
                        ([size, slots]) => (
                          <VehicleSlots
                            key={size}
                            name={size}
