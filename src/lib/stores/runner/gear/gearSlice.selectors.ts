...
/** `TState` for `ItemSelectors` below — file-local, same pattern as `AttrState` in
 ...
 * deliberately narrower than `RunnerData` — once 0015 Slice 5 moves `RunnerData.gear` to
 * `RunnerData._data_.items`, a caller just passes `{ items: runner._data_.items }` and nothing here
 * needs to change. Because `TState` no longer carries the whole Runner, these selectors can't
 * delegate to the legacy exports above (which need a full `RunnerData`) — they reimplement the
 * same, small filter/lookup logic directly against `ItemCatalog` instead. Existing exports and
 * call sites above are untouched.
 */
