...
export const BioSection: FC<BioSectionProps> = ({
  ...
  const showMetaTypeWarning =
    metatypes.length >= 1 && !metatypes.includes(bio.metatype)
    ...
    <Stack gap={1}>
      <Stack direction="row" gap={1} sx={{ flexWrap: "wrap" }}>
        ...
        <TextField
          ...
          value={bio.metatype}
          onChange={(event) =>
            onChange({ ...bio, metatype: event.target.value })
          }
          focused={showMetaTypeWarning}
          color={showMetaTypeWarning ? "warning" : undefined}
          helperText={
            showMetaTypeWarning
              ? "Metatype unavailable at current priority"
              : undefined
          }
        >
          {Object.keys(Metatypes).map((metatypeId) => (
            <MenuItem key={metatypeId} value={metatypeId}>
              {formatMetatype(metatypeId)}
            </MenuItem>
          ))}
        </TextField>
        ...
        <TextField
          ...
          value={bio.awakened}
          onChange={(event) =>
            onChange({ ...bio, awakened: event.target.value as AwakenedType })
          }
        ...
        >
          {Object.values(AwakenedType).map((awakened) => (
            <MenuItem key={awakened} value={awakened}>
              {awakened}
            </MenuItem>
          ))}
        </TextField>
      ...
      <Stack direction="row" gap={1} sx={{ flexWrap: "wrap" }}>
        <TextField
          ...
          value={bio.ethnicity}
          onChange={(event) =>
            onChange({ ...bio, ethnicity: event.target.value })
          }
        />
