...
const RemovableDeviceCard: FC<RemovableDeviceCardProps> = ({ deviceId }) => {
  const device = useRunnerSelector(ItemSelectors.selectById, { itemId: deviceId }) as DeviceData | undefined
...
describe("DeviceDataCard", () => {
  ...
  it("removing the device dispatches removeItem for it and its programs", async () => {
    ...
    // Assert
    await waitFor(() => expect(getItemCatalog(runnerStore.getState().runner)[deviceWithProgram.id]).toBeUndefined())
    expect(getItemCatalog(runnerStore.getState().runner)[runningProgram.id]).toBeUndefined()
  })
})
