...
describe("DataCardSlotDamageTrack", () => {
  it("forwards props to InlineDamageTrack", () => {
    const onChange = vi.fn()
    render(
      <DataCardSlotDamageTrack label="Damage" max={12} current={4} onChange={onChange} />,
  ...
  it("calls onChange when a cell is tapped", () => {
    const onChange = vi.fn()
    render(
      <DataCardSlotDamageTrack label="Damage" max={4} current={0} onChange={onChange} />,
      { wrapper: ThemeWrapper },
    )
