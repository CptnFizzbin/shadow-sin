...
describe("ItemCard", () => {
  it("dispatches weapons to WeaponItemCard", () => {
    renderWithProviders(<ItemCard item={weapon} />, { runnerStore: runnerStoreWithWeapon() })

    expect(screen.getByText("Ares Predator V")).toBeDefined()
    expect(screen.getByText("DV: 8P")).toBeDefined()
  })

  it("falls back to BasicItemCard for item types without a typed card", () => {
    const item: ItemData = {
  ...
  it("passes onOpen through to the rendered card", () => {
    const onOpen = vi.fn()
    renderWithProviders(<ItemCard item={weapon} onOpen={onOpen} />, {
