 ...
 * that BuilderState can use `Record<AttributeKey, number>` for attribute
 ...
 * result. The `value` field is stored directly on `BuilderState.attributes`
 * (as a plain number) while the limits are stored on `BuilderState.attributeLimits`.
 */
