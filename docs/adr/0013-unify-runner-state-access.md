...
[ADR-0014](./0014-selector-input-decomposition.md), which documents what this ADR got wrong and
what has to land first before its goal (below) can be safely attempted again.
...
## Postmortem (added on deprecation)

This ADR was implemented in full (#517/#518 — `useRunnerSelector` plus the `item`/`karmaCaps`/`magicAdvancement`/
`modifiers`/`skills`/`attributes`/`damage` catalogs, every call site migrated), reviewed, and merged, then
reverted the same cycle (#519) before it saw a second PR. Two independent problems, not one:

1. **Undiscovered dependency.** "One selector reused across Runner and every Entity kind" (the ambition behind
   `useAttrSelector`/`useMatrixSelector` treating attribute/damage access generically) had nothing real to
   unify over yet — no capability interface (`EntityWithAttrs`, `EntityWithDamage`, ...) existed for a Spirit,
   Sprite, or Vehicle to implement, so genuine cross-entity reuse wasn't actually available to build against;
   only Runner-shaped data existed. `docs/features/0015-entity-interface-decomposition.md` names this
   dependency and sequences it into shippable slices.
2. **The hook/catalog dispatch layer didn't earn its keep.** `useRunnerSelector(picker)` was meant to let
   multiple `useXSelector` hooks each advertise their own available-selectors catalog. In practice, each
   simplification pass on the review thread (#518) stripped ceremony the reviewer flagged — deprecate-on-
   creation selectors, ad hoc per-namespace shapes, inconsistent `by`/`for`/`of` naming — until what remained
   was barely distinguishable from calling `Selectors.<domain>.<fn>` directly through `useRunnerStoreSelector`,
   the exact status quo this ADR set out to replace. The dispatch layer added a real cost (an extra hook per
   domain, a `picker` indirection at every call site) for a benefit that kept evaporating under scrutiny.

`useRunnerSelector`/`useEntitySelector` (or their equivalents) remain the intended end state — this ADR isn't
wrong about the destination, only premature about building the dispatch mechanism before the data model could
back it up. See [ADR-0014](./0014-selector-input-decomposition.md) for the prerequisite step and what changes
before this ADR's approach can be revisited.
