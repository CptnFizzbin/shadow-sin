...
- [x] `createGear<T>()` factory in `gearData.ts`
...
- [x] `GearProvider` generic component — accepts any store with a `gear` field; used by both viewer and builder providers
...
- [ ] **Barrel / index export** for `src/components/Gear/` — add an `index.ts` so consumers import from `#/components/Gear` rather than individual file paths
